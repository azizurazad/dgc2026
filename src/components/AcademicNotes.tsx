...
export default function AcademicNotes({ notes, onDownloadNote }: AcademicNotesProps) {
  ...
  return (
    <section 
      id="academic-notes" 
      className="relative w-full py-24 md:py-32 bg-[#0B0B0B] text-[#F5F2EE] px-6 md:px-12 border-b border-[#C79A6B]/15"
    >
      <div className="w-full max-w-7xl mx-auto space-y-16">
        
        {/* SECTION HEADER */}
        <div className="flex flex-col lg:flex-row gap-8 items-start lg:items-end justify-between border-b border-[#C79A6B]/15 pb-8">
      ...
      {/* ==================================================
          ULTRA-PREMIUM DOCUMENT PREVIEW MODAL
          ================================================== */}
      <AnimatePresence>
        {selectedNoteForPreview && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#0B0B0B]/95 backdrop-blur-md">
            
            {/* Modal Backdrop animation wrapper */}
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}
              className={`w-full bg-[#111111] border border-[#C79A6B]/30 flex flex-col justify-between shadow-2xl relative transition-all duration-300 rounded-xs ${
                pdfFullscreen ? 'max-w-none h-screen m-0 border-0 rounded-none' : 'max-w-6xl h-[90vh]'
              }`}
            >
              
              {/* TOP HEADER STATUS BAR */}
              <div className="px-6 py-4 border-b border-[#C79A6B]/15 bg-[#0B0B0B] flex justify-between items-center gap-4 flex-wrap">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-9 h-9 bg-[#C79A6B]/10 border border-[#C79A6B]/25 flex items-center justify-center text-[#C79A6B]">
                    <FileText className="w-5 h-5" />
                  </div>
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-0.5 bg-[#C79A6B]/10 text-[#C79A6B] text-[8px] font-mono tracking-wider rounded uppercase">
                        {selectedNoteForPreview.category}
                      </span>
                      <span className="text-[9px] font-mono text-[#8F6A48]">{selectedNoteForPreview.subject}</span>
                    </div>
                    <h3 className="font-serif text-lg text-[#F5F2EE] truncate max-w-md mt-0.5">{selectedNoteForPreview.title}</h3>
                  </div>
                </div>

                {/* PDF VIEWER ACTION CONTROLS */}
                <div className="flex items-center gap-4 flex-wrap">
                  
                  {/* Page Navigator */}
                  <div className="flex items-center gap-2 bg-[#111111] px-3 py-1.5 border border-[#C79A6B]/10 rounded font-mono text-[10px] text-[#8F6A48]">
                    <button 
                      onClick={() => setPdfPage(prev => Math.max(1, prev - 1))}
                      disabled={pdfPage === 1}
                      className="hover:text-[#C79A6B] disabled:opacity-30 disabled:hover:text-[#8F6A48] cursor-pointer"
                    >
                      <ChevronLeft className="w-3.5 h-3.5" />
                    </button>
                    <span className="text-[#F5F2EE]">PAGE {pdfPage} / 4</span>
                    <button 
                      onClick={() => setPdfPage(prev => Math.min(4, prev + 1))}
                      disabled={pdfPage === 4}
                      className="hover:text-[#C79A6B] disabled:opacity-30 disabled:hover:text-[#8F6A48] cursor-pointer"
                    >
                  ...
                  <div className="flex items-center gap-2 bg-[#111111] px-3 py-1.5 border border-[#C79A6B]/10 rounded font-mono text-[10px] text-[#8F6A48]">
                    <button 
                      ...
                      className="hover:text-[#C79A6B] cursor-pointer"
                    >
                      <ZoomOut className="w-3.5 h-3.5" />
                    </button>
                    <span className="text-[#F5F2EE] min-w-[36px] text-center">{pdfZoom}%</span>
                    <button 
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="p-2 border border-[#C79A6B]/10 bg-[#111111] text-[#8F6A48] hover:text-[#C79A6B] rounded transition-all cursor-pointer"
                      title={pdfFullscreen ? "Exit Fullscreen" : "Fullscreen"}
                    >
                      <Maximize2 className="w-4 h-4" />
                    </button>
                    {selectedNoteForPreview.pdfUrl && selectedNoteForPreview.pdfUrl !== '#' && (
                      <a
                        href={selectedNoteForPreview.pdfUrl}
                        target="_blank"
                        rel="noreferrer"
                        className="p-2 border border-[#C79A6B]/10 bg-[#111111] text-[#8F6A48] hover:text-[#C79A6B] rounded transition-all flex items-center justify-center cursor-pointer"
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-[#C79A6B] text-[#0B0B0B] font-mono text-[10px] tracking-wider uppercase font-bold hover:bg-[#F5F2EE] transition-colors rounded-none cursor-pointer flex items-center gap-1.5"
                    >
                      <Download className="w-3.5 h-3.5" /> Download
                    </button>
                    <button
                      onClick={() => setSelectedNoteForPreview(null)}
                      className="p-2 border border-[#C79A6B]/20 bg-[#C79A6B]/5 text-[#C79A6B] hover:bg-[#C79A6B] hover:text-[#0B0B0B] rounded transition-all cursor-pointer"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </div>
              ...
              {/* MAIN CONTENT WORKSPACE (SPLIT VIEW FOR PREVIEW AND INDEXING) */}
              <div className="flex-grow flex flex-col md:flex-row overflow-hidden">
                
                {/* LEFT: Premium High-Fidelity PDF Canvas Simulator */}
                <div className="flex-grow bg-[#0B0B0B] p-6 overflow-auto flex items-center justify-center relative border-r border-[#C79A6B]/15 select-none">
                  <div 
                    style={{ transform: `scale(${pdfZoom / 100})`, transformOrigin: 'center center' }}
                    className="w-[100%] max-w-[650px] aspect-[1/1.41] bg-[#111111] border border-[#C79A6B]/30 shadow-2xl p-8 md:p-12 text-[#F5F2EE] flex flex-col justify-between transition-transform duration-300 relative overflow-hidden"
                  >
                    {/* Watermark / Academic Seal bg */}
                    <div className="absolute inset-0 flex items-center justify-center opacity-[0.03] pointer-events-none">
                      <FileText className="w-96 h-96 text-[#C79A6B]" />
                    </div>

                    {/* Sheet Header */}
                    <div className="flex justify-between items-start border-b border-[#C79A6B]/25 pb-4 text-left">
                      <div>
                        <p className="text-[10px] font-mono text-[#C79A6B] tracking-[0.2em] uppercase font-bold">DINAJPUR GOVT. COLLEGE</p>
                        <p className="text-[8px] font-mono text-[#8F6A48] uppercase mt-0.5">Department of Botany — Systematic Repository</p>
                      </div>
                      <div className="text-right">
                        <p className="text-[8px] font-mono text-[#8F6A48] uppercase">CLASS ARCHIVE</p>
                        <p className="text-[8px] font-mono text-[#C79A6B] uppercase font-semibold">{selectedNoteForPreview.session}</p>
                      </div>
                    </div>

                    {/* Page Content Rendering based on current page */}
                    {pdfPage === 1 && (
                      <div className="space-y-6 flex-grow py-10 flex flex-col justify-center text-left">
                        <span className="px-3 py-1 bg-[#C79A6B]/10 text-[#C79A6B] border border-[#C79A6B]/25 text-[9px] font-mono tracking-widest uppercase rounded self-start">
                          SECTION COVER
                        </span>
                        <h2 className="font-serif text-3xl font-light text-[#F5F2EE] leading-snug tracking-tight">
                          {selectedNoteForPreview.title}
                        </h2>
                        {selectedNoteForPreview.subtitle && (
                          <p className="font-mono text-sm italic text-[#C79A6B]/80 border-l border-[#C79A6B] pl-3">
                            {selectedNoteForPreview.subtitle}
                          </p>
                        )}
                        <p className="text-xs text-[#F5F2EE]/65 leading-relaxed font-light">
                          {selectedNoteForPreview.description}
                        </p>
                        <div className="grid grid-cols-2 gap-4 text-[9px] font-mono border-t border-dashed border-[#C79A6B]/20 pt-6">
                          <div>
                            <span className="text-[#8F6A48] uppercase tracking-wider block">CHIEF COMPILER:</span>
                            <span className="text-[#F5F2EE]">{selectedNoteForPreview.author}</span>
                          </div>
                          <div>
                            <span className="text-[#8F6A48] uppercase tracking-wider block">ACADEMIC SUPERVISOR:</span>
                            <span className="text-[#F5F2EE]">{selectedNoteForPreview.teacher}</span>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {pdfPage === 2 && (
                      <div className="space-y-4 flex-grow py-8 text-left">
                        <p className="text-[10px] font-mono text-[#C79A6B] tracking-widest uppercase border-b border-[#C79A6B]/10 pb-2">01. TAXONOMICAL TAXA KEY</p>
                        <p className="text-xs text-[#F5F2EE]/80 leading-relaxed">
                          The specimens cataloged under subject <span className="text-[#C79A6B]">{selectedNoteForPreview.subject}</span> follow rigorous morphological assessment standards. Leaf architectural grids and venation matrices remain key markers for verification.
                        </p>
                        <div className="p-4 bg-[#0B0B0B] border border-[#C79A6B]/10 rounded text-[10px] font-mono text-[#8F6A48] space-y-2">
                          <p className="text-[#C79A6B] font-bold">KEY INDICES USED:</p>
                          <p>• Leaf Blade Shape: Lanceolate to elliptical borders</p>
                          <p>• Apex Angle: Acute margin values with rich stomatal zones</p>
                          <p>• Venation: Brochidodromous veins with prominent tertiary grids</p>
                        </div>
                      </div>
                    )}
                    ...
                    {pdfPage === 3 && (
                      <div className="space-y-4 flex-grow py-8 text-left">
                        <p className="text-[10px] font-mono text-[#C79A6B] tracking-widest uppercase border-b border-[#C79A6B]/10 pb-2">02. METHODOLOGICAL APPROACH</p>
                        <p className="text-xs text-[#F5F2EE]/80 leading-relaxed">
                          For field expeditions cataloged in session <span className="text-[#C79A6B]">{selectedNoteForPreview.session}</span>, coordinates were logged via high-precision mapping tools. Dehydration of samples took place under a strictly regulated 60°C dry oven grid.
                        </p>
                        <div className="grid grid-cols-2 gap-2 text-[9.5px] font-mono text-[#8F6A48]">
                          <div className="p-2 border border-[#C79A6B]/10 rounded bg-[#0B0B0B]">
                            <span className="text-[#C79A6B] block">SEMESTER</span>
                            <span>{selectedNoteForPreview.semester}</span>
                          </div>
                          <div className="p-2 border border-[#C79A6B]/10 rounded bg-[#0B0B0B]">
                            <span className="text-[#C79A6B] block">VERIFICATION</span>
                    ...
                    {pdfPage === 4 && (
                      <div className="space-y-4 flex-grow py-8 text-left">
                        <p className="text-[10px] font-mono text-[#C79A6B] tracking-widest uppercase border-b border-[#C79A6B]/10 pb-2">03. BIBLIOGRAPHY & CITATIONS</p>
                        <p className="text-xs text-[#F5F2EE]/80 leading-relaxed">
                          The systematic curation references classic works like hooker's <em>Flora of British India</em> and modern angiosperm phylogenetic consensus databases.
                        </p>
                        <p className="text-[10px] text-[#8F6A48] font-mono">KEYWORDS: {selectedNoteForPreview.keywords.join(', ')}</p>
                        <div className="p-3 bg-[#161411] border border-[#C79A6B]/15 text-[9.5px] font-mono text-center text-[#C79A6B] uppercase font-bold">
                          *** END OF PREVIEW FILE ***
                        </div>
                      </div>
                    )}
                    ...
                    {/* Sheet Footer */}
                    <div className="flex justify-between items-center border-t border-[#C79A6B]/25 pt-4 text-[9px] font-mono text-[#8F6A48]">
                      <span>PAGE {pdfPage} OF 4</span>
                      <span>Verified: Department Curation Panel</span>
                      <span>{selectedNoteForPreview.fileSize}</span>
                    </div>

                  </div>
                </div>

                {/* RIGHT: Document Dossier side sidebar */}
                <div className="w-full md:w-[320px] bg-[#111111] p-6 overflow-y-auto space-y-6 flex-shrink-0 flex flex-col justify-between">
                  <div className="space-y-6 text-left">
                    {/* Heading details */}
                    <div className="space-y-2 border-b border-[#C79A6B]/15 pb-4">
                      <span className="text-[10px] font-mono text-[#8F6A48] uppercase tracking-wider block">DOCUMENT DOSSIER</span>
                      <h4 className="font-serif text-lg font-light text-[#F5F2EE] leading-tight">{selectedNoteForPreview.title}</h4>
                      <p className="text-xs font-mono text-[#C79A6B] uppercase tracking-widest">{selectedNoteForPreview.category}</p>
                    </div>

                    {/* Metadata specs */}
                    <div className="space-y-3.5 text-xs font-mono">
                      <div className="flex justify-between border-b border-[#C79A6B]/10 pb-2">
                        <span className="text-[#8F6A48] uppercase">Subject:</span>
                        <span className="text-[#F5F2EE]">{selectedNoteForPreview.subject}</span>
                      </div>
                      <div className="flex justify-between border-b border-[#C79A6B]/10 pb-2">
                        <span className="text-[#8F6A48] uppercase">Semester:</span>
                        <span className="text-[#F5F2EE]">{selectedNoteForPreview.semester}</span>
                      </div>
                      <div className="flex justify-between border-b border-[#C79A6B]/10 pb-2">
                        <span className="text-[#8F6A48] uppercase">Session:</span>
                        <span className="text-[#F5F2EE]">{selectedNoteForPreview.session}</span>
                      </div>
                      <div className="flex justify-between border-b border-[#C79A6B]/10 pb-2">
                        <span className="text-[#8F6A48] uppercase">Author Name:</span>
                        <span className="text-[#F5F2EE]">{selectedNoteForPreview.author}</span>
                      </div>
                      <div className="flex justify-between border-b border-[#C79A6B]/10 pb-2">
                        <span className="text-[#8F6A48] uppercase">Teacher:</span>
                        <span className="text-[#F5F2EE]">{selectedNoteForPreview.teacher}</span>
                      </div>
                      <div className="flex justify-between border-b border-[#C79A6B]/10 pb-2">
                        <span className="text-[#8F6A48] uppercase">File Size:</span>
                        <span className="text-[#F5F2EE]">{selectedNoteForPreview.fileSize}</span>
                      </div>
                    </div>

                    {/* Description */}
                    <div className="space-y-2 bg-[#0B0B0B] p-4 border border-[#C79A6B]/10 text-xs">
                      <span className="text-[10px] font-mono text-[#C79A6B] block uppercase">Scope & Summary:</span>
                      <p className="text-[#F5F2EE]/75 leading-relaxed font-light">{selectedNoteForPreview.description}</p>
                    </div>

                    {/* Keywords Tagging */}
                    <div className="space-y-2">
                      <span className="text-[10px] font-mono text-[#8F6A48] block uppercase">INDEXED KEYWORDS</span>
                      <div className="flex flex-wrap gap-1.5">
                        {selectedNoteForPreview.keywords.map(word => (
                          <span key={word} className="px-2 py-0.5 bg-[#0B0B0B] border border-[#C79A6B]/10 rounded text-[9px] font-mono tracking-wider text-[#8F6A48]">
                            #{word}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="border-t border-[#C79A6B]/15 pt-6 mt-6 text-left">
                    <p className="text-[10px] font-mono text-[#C79A6B] tracking-wider uppercase mb-3">RELATED REPOSITORIES</p>
                    <div className="space-y-2">
                      {relatedNotes.length === 0 ? (
                        <p className="text-[10px] font-serif italic text-[#8F6A48]">No adjacent index cards found</p>
                      ) : (
                        relatedNotes.map(n => (
                          <div 
                            key={n.id} 
                            onClick={() => handleOpenPreview(n)}
                            className="p-2 border border-[#C79A6B]/10 hover:border-[#C79A6B]/30 bg-[#0B0B0B] transition-colors rounded cursor-pointer"
                          >
                            <p className="text-xs font-serif text-[#F5F2EE] truncate">{n.title}</p>
                            <p className="text-[9px] font-mono text-[#8F6A48] uppercase mt-0.5">{n.category}</p>
                          </div>
                        ))
                      )}
                    </div>
